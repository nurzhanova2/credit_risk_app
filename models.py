...
def load_models() -> Dict[str, Any]:
    ...
    for name, path in MODEL_PATHS.items():
        try:
            ...
            print(f"[ПРЕДУПРЕЖДЕНИЕ] Файл модели {path} не найден. "
                  f"Модель '{name}' не будет использоваться.")
        except Exception as e:
            print(f"[ОШИБКА] Не удалось загрузить модель '{name}' из {path}: {e}")

    if not models:
        raise RuntimeError("Не удалось загрузить ни одной модели. "
...
def _dict_to_vector(client_data: dict) -> np.ndarray:
    ...
    for feat in FEATURE_NAMES:
        ...
        try:
            ...
            raise ValueError(f"Невозможно преобразовать '{feat}' "
